...
amberCheck=$(curl -u :$ADO_TOKEN "https://dev.azure.com/hmcts/$adoProject/_apis/build/builds?api-version=7.0&definitions=$adoPipelineDefinitionId&branchName=$branchName&resultFilter=succeeded&\$top=1&minTime=$isoTimeAmber" | jq -r .count)

redCheck=$(curl -u :$ADO_TOKEN "https://dev.azure.com/hmcts/$adoProject/_apis/build/builds?api-version=7.0&definitions=$adoPipelineDefinitionId&branchName=$branchName&resultFilter=succeeded&\$top=1&minTime=$isoTimeRed" | jq -r .count)
