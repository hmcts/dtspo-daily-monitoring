...
if [[ -z "${run}" ]]; then
    ...
    while read -r w; do
        ...
        while IFS=, read -r id name; do
            ...
            while IFS=, read -r workflow_status conclusion html_url run_started_at; do
                # clean up responses
                ...
                if [ -z ${workflow_status} ]; then
                    ...
                    elif [[ "${workflow_status}" == "waiting" ]] | [[ "${workflow_status}" == "pending" ]] | [[ "${workflow_status}" == "in_progress" ]] | [[ "${workflow_status}" == "queued" ]] | [[ "${workflow_status}" == "waiting" ]]; then
                        header
