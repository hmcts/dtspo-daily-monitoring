...
printf "> %s *%s* Open issues\n" "$OPEN_ISSUES_STATUS" "$OPEN_ISSUES_COUNT" >> slackmessage.txt
printf "> %s *%s* unassigned issues\n" "$UNASSIGNED_STATUS" "$UNASSIGNED_ISSUES_COUNT" >> slackmessage.txt

printf ">\n>\n>:tada:  *%s issues closed yesterday:* \n>\n" "$CLOSED_ISSUES_COUNT" >> slackmessage.txt
echo "${CLOSED_ISSUES_USER}">> slackmessage.txt


printf ">\n>\n>:fire: *Current Assigned tickets:* \n>\n" >> slackmessage.txt
echo "${ASSIGNED_ISSUES_RESULT}">> slackmessage.txt
