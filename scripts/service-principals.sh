...
if ! az login --service-principal --username "${b2c_sbox_serviceprincipal_id}" --password "${b2c_sbox_serviceprincipal_password}" --tenant "${b2c_sbox_tenant_id}" --allow-no-subscriptions; then
    echo "Failed to login to B2C tenant"
    exit 1
fi
...
if [ "$DOMAIN" = "hmctssboxextid.onmicrosoft.com" ]; then
    ...
    echo "Logging back into the original tenant using managed identity..."
