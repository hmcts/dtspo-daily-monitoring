...
if [ "${#resourcesInUnreadyState[@]}" -gt 0 ]; then
    slackThread+="\\n :red_circle: Postgres Server(s) in unready state! \\n$(IFS=$'\n'; echo "${resourcesInUnreadyState[*]}")\\n\\n"
fi

if [ "${#criticalCapacityResources[@]}" -gt 0 ]; then
    slackThread+="\\n :red_circle: Postgres Server(s) running above 95% storage capacity!: \\n$(IFS=$'\n'; echo "${criticalCapacityResources[*]}")\\n\\n"

fi

if [ "${#highCapacityResources[@]}" -gt 0 ]; then
    slackThread+="\\n :yellow_circle: Postgres Server(s) running above 80% storage capacity!: \\n$(IFS=$'\n'; echo "${highCapacityResources[*]}")\\n\\n"
fi
